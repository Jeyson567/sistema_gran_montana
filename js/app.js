...
function esEditable() {
  return estado === "editando" || (estado === "libre" && pedido.length === 0);
}
...
async function guardarMesa(mesaNombre) {
  ...
  if (pedido.length === 0) estadoFinal = "libre";
  else if (estado !== "enviado" && estado !== "listo") estadoFinal = "editando";
...
function actualizarEstadoVisual() {
  ...
  if (manualNombre) manualNombre.disabled = !editable;
  if (manualPrecio) manualPrecio.disabled = !editable;
  if (manualComentario) manualComentario.disabled = !editable;
  if (btnAgregarManual) btnAgregarManual.disabled = !editable;

  if (btnLimpiar) btnLimpiar.disabled = !editable;
  if (btnEnviarCocina) btnEnviarCocina.disabled = !editable || pedido.length === 0;
...
function renderizarPedidoHTML() {
  ...
  pedido.forEach((item, index) => {
    ...
    btnEliminar.disabled = !esEditable();

    btnEliminar.addEventListener("click", async () => {
      const mesaNombre = getMesaFromURL();
      if (!mesaNombre || !esEditable()) return;

      const confirmar = confirm(`¿Eliminar ${item.nombre}?`);
      if (!confirmar) return;

      pedido.splice(index, 1);
      if (pedido.length === 0) estado = "libre";
      else estado = "editando";
...
async function escucharMesaActual() {
  ...
  onSnapshot(doc(db, "mesas", mesaNombre), (snap) => {
    ...
    pedido = Array.isArray(datos.pedido) ? datos.pedido : [];
...
function crearCardCocina(datos, tipo) {
  ...
  totalBox.textContent = `Total: ${formatearQ(datos.total)}`;
  ...
  if (tipo === "enviado") {
    ...
    btn.onclick = async () => {
      await setDoc(
        doc(db, "mesas", mesaNombre),
        { estado: "listo", updatedAt: serverTimestamp() },
        { merge: true }
      );
    };
  ...
  if (tipo === "listo") {
    ...
    btn.onclick = async () => {
      await setDoc(
        doc(db, "mesas", mesaNombre),
        { estado: "enviado", updatedAt: serverTimestamp() },
        { merge: true }
      );
    };
    acciones.appendChild(btn);
...
function escucharCocina() {
  ...
  onSnapshot(collection(db, "mesas"), (snapshot) => {
    ...
    const conPedido = docs.filter((x) => Array.isArray(x.pedido) && x.pedido.length > 0);
    const enviados = conPedido.filter((x) => x.estado === "enviado");
    const listos = conPedido.filter((x) => x.estado === "listo");

    if (enviados.length === 0) {
      cocinaActiva.innerHTML = '<div class="cocina-vacia">No hay pedidos en cocina.</div>';
    } else {
      enviados.forEach((p) => cocinaActiva.appendChild(crearCardCocina(p, "enviado")));
    }

    if (listos.length === 0) {
      cocinaLista.innerHTML = '<div class="cocina-vacia">No hay pedidos listos.</div>';
    } else {
      listos.forEach((p) => cocinaLista.appendChild(crearCardCocina(p, "listo")));
    }
  });
}
...
if (btnAgregarManual) {
  btnAgregarManual.addEventListener("click", async () => {
    ...
    if (!esEditable()) {
      alert("No puedes agregar productos cuando el pedido ya fue enviado o está listo.");
      return;
    }
